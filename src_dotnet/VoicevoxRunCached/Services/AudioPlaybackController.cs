...
public class AudioPlaybackController : IDisposable
{
    ...
    /// <param name="cacheCallback">キャッシュ保存用コールバック（オプション）</param>
    /// <param name="cancellationToken">キャンセレーショントークン</param>
    /// <returns>再生完了を表すTask</returns>
    public async Task PlayAudioAsync(byte[] audioData, Func<byte[], Task>? cacheCallback = null, CancellationToken cancellationToken = default)
    {
        if (_disposed)
        ...
        try
        {
            ...
            // WavePlayerの作成
            _wavePlayer = _wavePlayerManager.CreateWavePlayer();
    ...
    /// <param name="cancellationToken">キャンセレーショントークン</param>
    /// <returns>再生完了を表すTask</returns>
    [Obsolete("Use PlayAudioAsync with optional cacheCallback parameter instead")]
    ...
    {
        await PlayAudioAsync(audioData, cacheCallback, cancellationToken);
    }
