...
public class CommandHandler
{
    ...
    public async Task<int> HandleTextToSpeechAsync(VoiceRequest request, bool noCache, bool cacheOnly, bool verbose = false, string? outPath = null, bool noPlay = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (verbose)
            {
                ConsoleHelper.WriteLine($"Engine check completed in {(DateTime.UtcNow - engineStartTime).TotalMilliseconds:F1}ms", _logger);
            }
            ...
            if (!noCache)
            {
                ...
                if (verbose)
                {
                    ConsoleHelper.WriteLine($"Segment processing completed in {(DateTime.UtcNow - segmentStartTime).TotalMilliseconds:F1}ms", _logger);
                }
                ...
                if (cachedCount > 0)
                {
                    ConsoleHelper.WriteSuccess($"Found {cachedCount}/{totalCount} segments in cache!", _logger);
                }
                ...
                if (uncachedSegments.Count > 0)
                {
                    if (cacheOnly)
                    {
                        ConsoleHelper.WriteError($"{uncachedSegments.Count} segments not cached and --cache-only specified", _logger);
                        return 1;
                    }

                    ConsoleHelper.WriteWarning($"Generating {uncachedSegments.Count} segments in background...", _logger);
                    generationTask = GenerateSegmentsAsync(request, segments, cacheManager, cancellationToken);
                }

                // Start playing immediately - cached segments play right away, uncached segments wait
                var playbackStartTime = DateTime.UtcNow;
                ConsoleHelper.WriteInfo("Playing audio...", _logger);
                using var audioPlayer = new AudioPlayer(_settings.Audio);
                var fillerManager = _settings.Filler.Enabled ? new FillerManager(_settings.Filler, cacheManager, _settings.VoiceVox.DefaultSpeaker) : null;
                await audioPlayer.PlayAudioSequentiallyWithGenerationAsync(segments, generationTask, fillerManager, cancellationToken);

                if (verbose)
                {
                    ConsoleHelper.WriteLine($"Audio playback completed in {(DateTime.UtcNow - playbackStartTime).TotalMilliseconds:F1}ms", _logger);
                }
            }
            else
            {
            ...
            ConsoleHelper.WriteSuccess("Done!", _logger);
        ...
        for (int i = 0; i < segments.Count; i++)
        {
            if (!segments[i].IsCached)
            {
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    {
                        ConsoleHelper.WriteDebug($"セグメントキャッシュがタイムアウトしました: {segmentRequest.Text}", _logger);
                    }
                    catch (Exception ex)
                    {
                        ConsoleHelper.WriteWarning($"セグメントのキャッシュに失敗しました: {segmentRequest.Text} - {ex.Message}", _logger);
                    }
                }, cancellationToken);
            }
        }
    ...
    private async Task WriteOutputFileAsync(byte[] wavData, string outPath, CancellationToken cancellationToken = default)
    {
        ...
        if (extension == ".mp3")
        {
            try
            {
                ...
                if (!isMp3)
                {
                    ...
                    ConsoleHelper.WriteWarning($"MP3 encoding fallback to WAV: {wavOut}", _logger);
                }
                else
                {
                    cancellationToken.ThrowIfCancellationRequested();
        ...
        if (extension != ".wav")
        {
            ...
            ConsoleHelper.WriteWarning($"Adjusted extension to .wav: {corrected}", _logger);
        }
        else
        {
            cancellationToken.ThrowIfCancellationRequested();
