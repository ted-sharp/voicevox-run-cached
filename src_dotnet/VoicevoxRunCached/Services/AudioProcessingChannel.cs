...
public class AudioProcessingChannel : IDisposable
{
    ...
    public async Task<AudioProcessingResult> ProcessAudioAsync(VoiceRequest request, CancellationToken cancellationToken = default)
    {
        ...
        // まずキャッシュをチェック
        var cachedAudio = await _cacheManager.GetCachedAudioAsync(request);
        if (cachedAudio != null)
        {
        ...
        try
        {
            ...
            var audioQuery = await _apiClient.GenerateAudioQueryAsync(request, cancellationToken);
            var audioData = await _apiClient.SynthesizeAudioAsync(audioQuery, request.SpeakerId, cancellationToken);

            // キャッシュに保存
            await _cacheManager.SaveAudioCacheAsync(request, audioData);
